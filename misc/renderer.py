...
def is_monotonic(sequence):
    return torch.all(sequence[1:] > sequence[:-1]) or torch.all(sequence[1:] < sequence[:-1])

def are_nearby(proj_pt_sequence, pixel_threshold=5):
    for i in range(len(proj_pt_sequence)-1):
        for j in range(i+1, len(proj_pt_sequence)):
            if torch.norm(proj_pt_sequence[i] - proj_pt_sequence[j], p=2) <= pixel_threshold:
                return True
    return False

def prune_based_on_viewpoint(point_cloud, intrinsics, extrinsics, image_shape, k=16, density_threshold=0.5):
    # Calculate local densities
    distances = pytorch3d.ops.knn_points(point_cloud.unsqueeze(0), point_cloud.unsqueeze(0), K=k+1)[1]
    densities = 1.0 / distances[:, :, 1:].mean(dim=-1).squeeze()
    low_density_mask = densities <= density_threshold

    # Project points to image space
    visible_points, proj_pts = pts_3d_to_img_raster(point_cloud, intrinsics, extrinsics, image_shape)
    
    # Further refine visible points based on low density
    low_density_visible_points = visible_points[low_density_mask]
    low_density_proj_pts = proj_pts[low_density_mask]

    # Sort by depth to help with monotonic check
    _, sorted_idxs = torch.sort(low_density_visible_points[:, 2])
    sorted_low_density_visible_points = low_density_visible_points[sorted_idxs]
    sorted_low_density_proj_pts = low_density_proj_pts[sorted_idxs]

    # Check for low-density points that project to nearby locations with monotonic depth
    removal_mask = torch.zeros_like(densities, dtype=torch.bool)
    for i in range(len(sorted_low_density_visible_points) - 2):
        # Check monotonicity in depth
        if is_monotonic(sorted_low_density_visible_points[i:i+3, 2]):
            # Check if projected points are nearby
            if are_nearby(sorted_low_density_proj_pts[i:i+3]):
                removal_mask[sorted_idxs[i:i+3]] = True

    # Apply inverse mask to keep points not marked for removal
    pruned_cloud = point_cloud[~removal_mask]
