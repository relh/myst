...
def create_outpainting_image_and_mask(image, zoom):
    ...
    new_h, new_w = transformed_image_tensor.shape[2:]
    pad_h = (max(new_h, new_w) - new_h) // 2
    pad_w = (max(new_h, new_w) - new_w) // 2

    # Apply padding to maintain border effect
    padded_image_tensor = pad(transformed_image_tensor, [pad_w, pad_h, pad_w, pad_h], padding_mode="edge")
    ...
    padded_image_array = padded_image_tensor.squeeze().permute(1, 2, 0).numpy()

    # Ensure correct data type and range for PIL compatibility
